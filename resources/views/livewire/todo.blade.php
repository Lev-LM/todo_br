<div>
   ...
   {{-- First todo --}}
    ...
    <div>
    <input id="green-checkbox" wire:click='markerCompl({{ $todo->id }})' @if($todo->is_completed) checked @endif type="checkbox" class="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
    </div>
    <div>
    @if ($edit == $todo->id)
    ...
    @else
    <x-secondary-button color="green" wire:click='editTodo({{ $todo->id }})'>
    Edit
    </x-secondary-button>
    @endif
    </div>
